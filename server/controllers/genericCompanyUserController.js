...
genericCompanyUserController.getUserTickets = async (req, res) => {
  try {
    const userTicketInfo = await db.query(
      'SELECT * FROM tickets JOIN Users ON tickets.user_id = users.id WHERE users.id=$1;',
      [req.body.userId]
    );
...
genericCompanyUserController.updateUserTicket = async (req, res) => {
  try {
    ...
    const { userID, details, ticketStatus, ticketID } = req.body;
    const updateTicket = await db.query(
      'UPDATE tickets SET details=$1, status=$2 WHERE (tickets.user_id=$3 AND tickets.id=$4);',
      [details, ticketStatus, userID, ticketID]
    );
...
genericCompanyUserController.deleteUserTicket = async (req, res) => {
  try {
    ...
    const deleteUserTicket = await db.query(
      'DELETE FROM tickets WHERE (tickets.id=$1 AND tickets.user_id=$2);',
      [ticketID, userID]
    );
...
genericCompanyUserController.createTicket = async (req, res) => {
  ...
  const createdUserTicket = await db.query(
    'INSERT INTO tickets (user_id, details) VALUES ($1, $2);',
    [userID, details]
  );
